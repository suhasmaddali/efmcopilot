...
from src.prompts_new_test import (
...
# Custom formatting function for bar labels
def format_custom_ticks(value):
    if abs(value) >= 1e9:  # Billion
...
def content_page():
    ...
    with st.expander("Click to view the data", expanded=True):
        ...
        file_uploaded = st.file_uploader("Upload your BUF data file:", type=["csv"])
        ...
        if file_uploaded:
            ...
            try:
                ...
                st.write("Currently the app accepts BUF SELLIN data in a particular format. Thanks.")
    ...
    if flag == 1:
        ...
        system_message = """Looks like the file is not uploaded yet. Let the user know that the file has not been uploaded yet"""
        st.session_state.messages[0] = {"role": "system", "content": system_message}
    ...
    if prompt := st.chat_input("Write your lines here..."):
        ...
        if flag == 1:
            ...
            additional_message = f"""
            Let the user who has prompted know data has not yet been uploaded yet
            """
